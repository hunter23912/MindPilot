...
# Put Hugging Face cache on the data disk to avoid filling / with temporary download files
export HF_HOME=/root/autodl-tmp/hf
export HF_HUB_CACHE=/root/autodl-tmp/hf/hub
export HF_XET_CACHE=/root/autodl-tmp/hf/xet

# Download all files
source /etc/network_turbo # 设置服务器网络代理

HF_ENDPOINT=https://hf-mirror.com \
HF_HUB_ENABLE_HF_TRANSFER=1 \
HF_HUB_DISABLE_FILE_LOCKING=1 \
hf download LidongYang/Mindpilot --repo-type dataset --local-dir ./data
```

The `--local-dir ./data` target stores the dataset files under your current project directory, while Hugging Face still keeps metadata and temporary cache files under `HF_HOME`. On Autodl, setting these cache paths to `/root/autodl-tmp` prevents the 30 GB system disk from being exhausted during large downloads.
