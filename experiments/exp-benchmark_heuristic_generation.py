...
def visualize_top_images(images, similarities, save_path):
...
def run_single_experiment(method, target_idx, seed, config, vlmodel, preprocess_train, pipe):
    ...
    encoding_model_path = get_env_value(
        "IMG2EEG_MODEL_PATH",
        default=os.path.join(str(get_project_root()), "data", "encoding_models_checkpoints", sub, "synthetic_eeg_data", "encoding-end_to_end", f"dnn-{dnn}", "modeled_time_points-all", "pretrained-True", "lr-1e-05__wd-0e+00__bs-064", "model_state_dict.pt"),
    )
    f_encoder = get_env_value(
        "EEG2CLIP_ENCODING_MODEL_PATH",
        default=os.path.join(str(get_project_root()), "data", "sub_model", sub, "diffusion_250hz", "ATM_S_reconstruction_scale_0_1000_40.pth"),
    )
    ...
    for t in range(num_loops):
        ...
        if method == "random_generation":
            ...
            if t == 0:
                ...
                for fusion_idx in range(NUM_FUSIONS_PER_ROUND):
                    ...
                    generated_images, (idx1, idx2) = fusion_image_to_images(Generator, img_embeds, fit_rewards, device, 512)
        ...
        # Visualize
        visualize_top_images(loop_sample_ten, loop_reward_ten, os.path.join(exp_save_dir, f'loop{t + 1}.png'))
    ...
    for i, img in enumerate(final_images):
        save_name = os.path.join(final_save_dir, f"final_generated_{i}.png")
        img.save(save_name)
...
def main():
    ...
    test_features_path = get_env_value(
        "MINDPILOT_TEST_IMAGE_FEATURES",
        default="",
    )
    if test_features_path and os.path.exists(test_features_path):
        test_set_img_embeds = torch.load(test_features_path, weights_only=False)['img_features'].cpu()
    else:
        # Fallback: compute from individual embed files
        print("  Test image features file not found, computing from individual embed files...")
        embeds_list = []
        for embed_file in embed_list:
            embed_path = os.path.join(embed_dir, embed_file)
            embed = torch.load(embed_path, weights_only=False)
            if isinstance(embed, dict):
                embed = embed.get('img_features', embed.get('embed', embed[list(embed.keys())[0]]))
            embeds_list.append(embed.cpu())
        test_set_img_embeds = torch.stack(embeds_list).squeeze(1)
    ...
    config = {
        ...
        'save_path': get_env_value(
            "BENCHMARK_HEURISTIC_OUTPUT_DIR",
            default=os.path.join(str(get_project_root()), "benchmark_results_total", "benchmark_heuristic_generation"),
        ),
