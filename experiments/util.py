...
proxy = get_env_value("MINDPILOT_PROXY", default="")
if proxy:
...
def preprocess_image(image_path, device):
    return _preprocess_image(image_path, device)
...
def generate_eeg(model, image_tensor, device):
    ...
    eeg_output = model(image_tensor)
    # Assuming model output is a (1, 1700) vector, reshape it to (17, 100)
    eeg_output = eeg_output.reshape((17, 250))
...
def save_eeg_signal(eeg_signal, save_dir, idx, category):
    ...
    file_name = f"{category}_{idx + 1}.npy"  # Filename includes sorting identifier
...
def plot_similarity_range(similarities_per_iteration, save_folder):
    ...
    # Calculate mean, max, and min similarity per iteration
    ...
    # Plot error band (min-max range band)
    ...
    plt.savefig(save_path, bbox_inches='tight', dpi=300)  # Save image file in high resolution
...
def save_value_function_to_txt(value_function, save_folder,iteration):
    ...
    # Write values to .txt file in rows (12 per row)
    with open(save_path, 'w') as f:
        for i in range(0, len(value_function), 12):
            line_values = value_function[i:i + 12]
            line_str = " ".join(map(str, line_values))  # Separate values with spaces
...
def get_image_path(category_idx, image_idx, text_list):
    """
    Return the image path given category and image indices; checks if folder contains 12 images.
    :param category_idx: Category index
    :param image_idx: Image index
    :param text_list: List of folder names
    ...
    # print(image_file)
        # # Check if the folder contains 12 images
    ...
    # # Get image filename, assuming images in the folder are sorted in some order
...
def load_thingstestimagedata(img_directory):
    images = []
    category_images = []  # Store 12 images per category
    ...
    for folder in all_folders:
        ...
        folder_images.sort()  # Ensure images are sorted within each category
        # print(folder_images)
...
# Filename includes sorting identifier
    gt_eeg_path = os.path.join(gt_eeg_path, file_name)
    np.save(gt_eeg_path, synthetic_eeg.cpu())
    return gt_eeg_path



    
def img_enc_eeg(image_tensor, encoder_model_path, dnn, device):
...
def plot_similarity_and_mse_with_dual_axis(similarities_per_iteration, save_folder, target_similarity=1.0, title='Similarity_and_MSE_Dual_Axis.png'):
    """
    Plot similarity and MSE curves with error bands using dual y-axes, with adjusted border line widths.
    ...
    Args:
        ...
        target_similarity (float, optional): Target similarity for computing MSE, default 1.0 (perfect similarity)
    """
    # Data preprocessing
    ...
    # Calculate MSE (based on max_similarities)
...
def visualize_images(image_paths, save_folder, iteration):
    ...
    :param image_paths: List of image paths
    :param similarities: List of similarities for each image
    """
...
def visualize_top_images(image_paths, similarities, save_folder, iteration):
    ...
    :param similarities: List of similarities for each image
    """
    # Combine image paths with similarities and sort by similarity in descending order
...
def extract_number(filename):
    ...
    if numbers:
        return tuple(map(int, numbers))  # Return a tuple of multiple numbers
    return (float('inf'),)  # If no numbers found, return a tuple with infinity

    # match = re.search(r'(\d+)', filename)
    # return int(match.group(1)) if match else 0

# Iterate over image folder and process each image
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        # Compute an extra element to handle odd dimensions
...
def load_model_decoder(model_path, device):
    """
    Load pretrained EEG and image models along with the optimizer state.
    """
    checkpoint = torch.load(model_path, map_location=device)
    eeg_model = ATM_S_reconstruction_scale_0_1000(17, 250)  # e.g., ATM_S_reconstruction_scale_0_1000 is the EEG model
    img_model = Proj_img()  # Assuming Proj_img model is used
...
def evaluate_eeg_signals(eeg_model, img_model, eeg_signals_truth, device, truth_folder, false_folder, truth, false):
    """
    Classify the given EEG signals and compute accuracy.
    """
    correct = 0
    total = 0
    correct_samples = []
    ...
    with torch.no_grad():
        # Iterate over EEG signals from truth and false folders
        for idx, eeg_data in enumerate(eeg_signals_truth):
            ...
            # Merge truth and false logits to determine classification correctness
        ...
        # Iterate over EEG signals from the false folder
        ...
        #     if predicted_label == true_label:
        #         correct += 1
        #         correct_samples.append(idx + len(eeg_signals_truth))  # Index offset by truth count
