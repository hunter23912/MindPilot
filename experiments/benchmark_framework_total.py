...
for extra_path in (PROJECT_ROOT, PROJECT_ROOT / "model", PROJECT_ROOT / "experiments"):
...
class PseudoModelWrapper(BaseMethod):
    ...
    def __init__(self, config, device="cuda", shared_models=None):
        ...
        if shared_models is not None:
            ...
            self.pipe = shared_models.get('pseudo_pipe') or shared_models.get('sdxl_pipe')
        else:
            # Standalone mode: load own models
            print("  Loading independent models...")
            from exp_batch_offline_generation import DummyVLModel, DummyPreprocess
            from model.custom_pipeline_low_level import Generator4Embeds
            self.vlmodel = DummyVLModel().to(device).eval()
            self.preprocess_train = DummyPreprocess()
            self.pipe = Generator4Embeds(device=device).pipe
        
        # Load models using base class methods
        print("  Loading EEG model...")
        self.eeg_model = self._load_eeg_model(config['EEG2CLIP_ENCODING_MODEL_PATH'])
        ...
        load_ip_adapter = shared_models is None
...
class HeuristicClosedLoopWrapper(BaseMethod):
    ...
    def __init__(self, config, device="cuda", shared_models=None):
        ...
        # Use the dedicated pseudo-model pipe when available; otherwise fall back to the shared SDXL pipe
        if shared_models is not None and (shared_models.get('pseudo_pipe') is not None or shared_models.get('sdxl_pipe') is not None):
            self.pipe = shared_models.get('pseudo_pipe') or shared_models.get('sdxl_pipe')
        else:
            from model.custom_pipeline_low_level import Generator4Embeds
            self.pipe = Generator4Embeds(device=device).pipe
        ...
        # Shared runs reuse the preconfigured pipe; standalone runs load the adapter here.
        self.generator = HeuristicGenerator(
            self.pipe, 
            self.vlmodel, 
            self.preprocess_train, 
...
class BenchmarkFramework:
    ...
    def __init__(self, config_path: str):
        ...
        self.config = resolve_config_values(self.config)
        
        self.device = self.config.get('device', 'cuda')
        if self.device.startswith('cuda') and not torch.cuda.is_available():
            print("WARNING: CUDA was requested but is unavailable; falling back to CPU.")
            self.device = 'cpu'
        self.output_dir = self.config.get('output_dir') or os.path.join(str(PROJECT_ROOT), 'benchmark_results', 'benchmark_framework_total')
    ...
    def _load_shared_models(self):
        ...
        for method_name in enabled_methods:
            ...
            if 'EEG2CLIP_ENCODING_MODEL_PATH' in config and 'encoding_model_path' in config:
        ...
        checkpoint = torch.load(
            sample_config['EEG2CLIP_ENCODING_MODEL_PATH'], 
        ...
        if 'pseudo_model' in enabled_methods:
            ...
            from exp_batch_offline_generation import DummyVLModel, DummyPreprocess
            vlmodel = DummyVLModel().to(self.device).eval()
            preprocess_train = DummyPreprocess()
            shared_models['vlmodel'] = vlmodel
            shared_models['preprocess_train'] = preprocess_train

        if any(m in enabled_methods for m in ['pseudo_model', 'heuristic_closedloop']):
            print("  Loading shared pseudo-model SDXL pipeline...")
            from model.custom_pipeline_low_level import Generator4Embeds
            pseudo_pipe = Generator4Embeds(device=self.device).pipe
            pseudo_pipe.load_ip_adapter(
                "h94/IP-Adapter",
                subfolder="sdxl_models",
                weight_name="ip-adapter_sdxl_vit-h.bin",
                torch_dtype=torch.bfloat16,
            )
            pseudo_pipe.set_ip_adapter_scale(0.5)
            shared_models['pseudo_pipe'] = pseudo_pipe
    ...
    def _create_shared_sdxl_pipeline(self, device):
        ...
        vae = AutoencoderKL.from_pretrained(
            "madebyollin/sdxl-vae-fp16-fix", torch_dtype=torch.bfloat16, local_files_only=True
        )
        
        scheduler = DDIMScheduler.from_pretrained(
            model_id, subfolder="scheduler", timestep_spacing="trailing", local_files_only=True
        )
        ...
        try:
            ...
            if result['best_image'] is not None:
                save_dir = os.path.join(
                    self.output_dir, 'images', method.name
                )
                os.makedirs(save_dir, exist_ok=True)
                result['best_image'].save(
                    os.path.join(save_dir, f"target_{target_idx}_seed_{seed}.png")
                )
