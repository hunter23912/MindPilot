...
sys.path.insert(0, str(get_project_root() / 'model'))
...
model_type = 'ViT-H-14'
local_open_clip_weights = get_env_value("OPEN_CLIP_WEIGHTS", default="")
if local_open_clip_weights and os.path.exists(local_open_clip_weights):
    print(f"Loading OpenCLIP from local weights: {local_open_clip_weights}")
    vlmodel, preprocess_train, feature_extractor = open_clip.create_model_and_transforms(
        model_type, pretrained=None, precision='fp32', device=device)
    model_state_dict = torch.load(local_open_clip_weights, map_location=device, weights_only=True)
    vlmodel.load_state_dict(model_state_dict)
else:
    print("Local OpenCLIP weights not found, falling back to Hugging Face download.")
    vlmodel, preprocess_train, feature_extractor = open_clip.create_model_and_transforms(
...
image_dir = get_env_value(
    "TEST_IMAGE_DIR",
    default=os.path.join(project_root, "data", "test_images"),
)
embed_dir = get_env_value(
    "CLIP_EMBED_DIR",
    default=os.path.join(project_root, "data", "things-eeg2", "clip_image_embeds"),
)

if not os.path.isdir(image_dir):
    raise FileNotFoundError(
        f"Image gallery directory not found: {image_dir}. "
        "Download the MindPilot dataset from Hugging Face into ./data, "
        "or set TEST_IMAGE_DIR to your local image folder."
    )

if not os.path.isdir(embed_dir):
    raise FileNotFoundError(
        f"CLIP image embed directory not found: {embed_dir}. "
        "Run `python experiments/generate_clip_image_embeds.py` to generate it first. "
    )
...
class HeuristicGenerator:
    def __init__(self, pipe, vlmodel, preprocess_train, device="cuda", seed=0):
        ...
        # Load IP adapter (use the exact same code as first script, no explicit attn processor reset)
        ...
        # NOTE: No explicit device placement of text_encoder/image_encoder/unet,
        # letting diffusers handle it as in the first script.
...
def run_single_experiment(method, target_idx, seed, config):
    ...
    # Load EEG-to-CLIP model (ATMS: maps EEG signals to CLIP feature space)
    sub = 'sub-01'
    f_encoder = get_env_value("EEG2CLIP_ENCODING_MODEL_PATH")
    ...
    # Image-to-EEG model (AlexNet-based encoder: maps images to synthetic EEG signals)
    encoding_model_path = get_env_value("IMG2EEG_MODEL_PATH")
    ...
    if method == "eeg_guidance":
        ...
        # Generate using empty string prompt, exactly as in the first script
    ...
    save_dir = os.path.join(config['save_path'], method)
    os.makedirs(save_dir, exist_ok=True)

    saved_image_paths = []
    final_images_copy = []
    for i, img in enumerate(final_images):
        save_name = os.path.join(save_dir, f"target_{target_idx}_seed{seed}_{i}.png")
        img.save(save_name)
...
def main():
    ...
    config = {
        'target_indices': [i for i in range(0, 200, 8)],
        'num_seeds': 1,  # Number of random seeds per method
        'offline_batch_size': 10,  # Offline data batch size
        'save_path': get_env_value(
            "BENCHMARK_OFFLINE_OUTPUT_DIR",
            default=os.path.join(str(get_project_root()), "benchmark_results", "benchmark_offline_generation"),
        ),
