...
from model.subject_layers.Transformer_EncDec import Encoder, EncoderLayer
from model.subject_layers.SelfAttention_Family import FullAttention, AttentionLayer
from model.subject_layers.Embed import DataEmbedding
import numpy as np
...
class PositionalEncoding(nn.Module):
    """Standard sinusoidal positional encoding, matches old checkpoint pos_encoder."""
...
class LegacyAttentionModel(nn.Module):
    """Matches the old checkpoint architecture: pos_encoder + encoder_layer + transformer_encoder."""
    def __init__(self, d_model=17, nhead=3, dim_feedforward=2048, max_len=5000):
        super().__init__()
...
# Attention-based Temporal Modeling for EEG-based Image Retrieval (ATMS)
class ATMS(nn.Module):
    def __init__(self, num_channels=17, sequence_length=250, num_subjects=1, num_features=17, num_latents=1024, num_blocks=1):
        ...
        self.attention_model = LegacyAttentionModel(d_model=num_features, nhead=1, dim_feedforward=2048, max_len=5000)
    ...
    def forward(self, x, subject_ids):
        # x: (batch, 17, seq_len) from synthetic EEG
        x = x.permute(0, 2, 1)  # (batch, seq_len, 17)
        x = self.attention_model(x)  # (batch, seq_len, 17)
        x = x.permute(0, 2, 1)  # (batch, 17, seq_len)
        x = self.subject_wise_linear[0](x)  # (batch, 17, seq_len)
