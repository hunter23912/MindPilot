...
def generate_ip_adapter_embeds(
    ...
    img2img_strength: float = 0.85,  # img2img denoising strength
    low_level_image: Optional[PipelineImageInput] = None,  # low-level reference image
    ...
    **kwargs,
    ...
    # Process low_level_image
    if low_level_image is not None:
        # Ensure low_level_image is converted to the appropriate format
    ...
    # Calculate initial step count based on img2img_strength
    if low_level_image is not None:
        ...
        t_start = 0  # Start from scratch if no low_level_image is provided
    ...
    if low_level_image is not None and low_level_latent is None:
        # Encode low_level_image into latent space
        ...
        # Randomly initialize latents if no low_level_image is provided
        latents = self.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
...
class Generator4Embeds:
    ...
    def __init__(self, num_inference_steps=1, device='cuda', img2img_strength=1, low_level_image=None, low_level_latent = None) -> None:
        ...
        pipe = ExtendedStableDiffusionXLPipeline.from_pretrained(
            model_id,
            unet=unet,
            vae=vae,
            scheduler=scheduler,
