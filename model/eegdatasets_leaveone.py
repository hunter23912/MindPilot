...
model_weights_path = get_env_value("OPEN_CLIP_WEIGHTS", default="")
img_directory_training = get_env_value("MINDPILOT_THINGS_TRAIN_DIR", default="")
img_directory_test = get_env_value("THINGS_EEG2_TEST_IMAGE_DIR", default="")
FEATURES_TRAIN_PATH = get_env_value("MINDPILOT_FEATURES_TRAIN_PATH", default="")
FEATURES_TEST_PATH = get_env_value("MINDPILOT_FEATURES_TEST_PATH", default="")
vlmodel = None
...
def _ensure_vlmodel():
    global vlmodel, preprocess_train, feature_extractor
    if vlmodel is None or preprocess_train is None or feature_extractor is None:
        import open_clip
...
# config_path = "/mnt/dataset0/ldy/Workspace/FLORA/data_preparing/data_config.json"
...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        ...
        if not os.path.isdir(self.data_path):
            raise RuntimeError(f"EEG data directory does not exist: {self.data_path}")
        ...
        if not img_directory_training:
            raise RuntimeError("Set MINDPILOT_THINGS_TRAIN_DIR before constructing EEGDataset.")
        if not img_directory_test:
            raise RuntimeError("Set THINGS_EEG2_TEST_IMAGE_DIR before constructing EEGDataset.")
        if not FEATURES_TRAIN_PATH:
            raise RuntimeError("Set MINDPILOT_FEATURES_TRAIN_PATH before constructing EEGDataset.")
        if not FEATURES_TEST_PATH:
            raise RuntimeError("Set MINDPILOT_FEATURES_TEST_PATH before constructing EEGDataset.")
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = self.features_train_path if self.train else self.features_test_path
    ...
    def Textencoder(self, text):   
            vlmodel_local, _, _ = _ensure_vlmodel()
            import open_clip
            ...
            with torch.no_grad():
                text_features = vlmodel_local.encode_text(text_inputs)
    ...
    def ImageEncoder(self,images):
        vlmodel_local, preprocess_train_local, _ = _ensure_vlmodel()
        batch_size = 20  
        image_features_list = []
      
        for i in range(0, len(images), batch_size):
            batch_images = images[i:i + batch_size]
            image_inputs = torch.stack([preprocess_train_local(Image.open(img).convert("RGB")) for img in batch_images]).to(device)

            with torch.no_grad():
                batch_image_features = vlmodel_local.encode_image(image_inputs)
...
if __name__ == "__main__":
    ...
    # data_path should point to the root of the preprocessed EEG dataset.
    data_path = get_env_value("MINDPILOT_EEG_DATA_PATH", default="")
    if not data_path:
        raise RuntimeError("Set MINDPILOT_EEG_DATA_PATH before running model/eegdatasets_leaveone.py directly.")
